...
function UniteCreatorParamsPanel(){
	...
	function addImageParam(objParam, filter){
		...
		if(isAddThumbSizes == true){
			...
			//addParam.type = null;
			var objParam1 = {};
			objParam1.name = objParam.name;	
			
			objParam1.is_child = g_ucAdmin.getVal(objParam, "is_child");
			if(objParam1.is_child === true){
				objParam1.parent_name = g_ucAdmin.getVal(objParam, "parent_name");
			}
			
			addParam(objParam1, "uc_textfield", filter);
		}
						
		//add child sizes
		if(isAddThumbSizes == false){		//for posts images
			
			if(isAddThumb){
				objParamThumb.tooltip = "Thumb";
				addParam(objParamThumb, "uc_textfield", filter);
			}	
			
			var tooltopLarge = g_ucAdmin.getVal(g_objThumbSizes, "large");
			if(!tooltopLarge)
				tooltopLarge = "Large";
			
			if(isAddThumbLarge){
				objParamThumb.name = name+"_thumb_large";
				objParamThumb.tooltip = tooltopLarge;
				addParam(objParamThumb, "uc_textfield", filter);
			}
			
			jQuery.each(g_objThumbSizes, function(size, sizeTitle){
				
				if(isAddThumbLarge == true && size == "large")
					return(true);
			...
			//add additional params (alt, title)
			
			if(g_objImageAddParams){
				
				jQuery.each(g_objImageAddParams, function(index, paramName){
					
					objParamThumb.name = name+"_"+paramName;
					objParamThumb.tooltip = null;
					objParamThumb.type = "uc_textfield";
