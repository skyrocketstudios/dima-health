...
";
  ...
  public static function getPixelTrackCode($event, $param = array(), $tracking_name = '', $with_script_tag = true) {
    ...
    $theevent = "";
    if($currentpage == 7) { /* cart page */
        $theevent = "AddToCart";
        $code = "<script type='text/javascript'> fbq('track', '". $theevent  ."') </script>";

    } else if($currentpage == 8) { /* checkout page */
        $theevent = "Purchase";
        
        $code = "<script type='text/javascript'> fbq('track', '". $theevent  ."',{value:0.00,currency:'PHP'}) </script>";
  ...
  public static function getPixelNoscriptCode($event = 'PageView', $cd = array(), $tracking_name = '') {
    ...
    if($currentpage == 7) { /* cart page */
        ...
        <noscript>
        <img height=\"1\" width=\"1\" style=\"display:none\" alt=\"fbpx\"
        src=\"https://www.facebook.com/tr?id=%s&ev=".  $theevent ."&noscript=1\" />
        </noscript>
        <!-- End Facebook Pixel Code -->
        ";
