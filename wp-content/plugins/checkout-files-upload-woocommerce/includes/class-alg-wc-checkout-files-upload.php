...
class Alg_WC_Checkout_Files_Upload_Main {
		...
		for ( $i = 1; $i <= $total_files; $i++ ) {
			...
			if ( '' != $order_file_name ) {
				...
				$html .= '<p><a href="' . add_query_arg(
					...
					) ) . '" target="_blank">' . $real_file_name . '</a></p>';
			}
		}
		if ( ! $files_exists ) {
	...
	function process_checkout_files_upload() {
		...
		if ( isset( $_GET['alg_download_checkout_file_admin'] ) ) {
			...
			if ( alg_is_user_role( 'administrator' ) || alg_is_user_role( 'shop_manager' ) || alg_is_user_role( 'orders_manager' ) ) {
				// header( "Expires: 0" );
				...
				//     case "svg": $ftype="image/svg+xml"; break;
				//     default:
				// }
				// header('Content-type: ' . $ftype);
				// readfile( $tmp_file_name );
				$file = $_GET['alg_download_checkout_file_admin'];
				echo "<div class='img-container-prescription'>";
				echo "<img src='../wp-content/uploads/woocommerce_uploads/alg_uploads/checkout_files_upload/". $file ."' />";
				exit();
			}
		}
		// User file download
