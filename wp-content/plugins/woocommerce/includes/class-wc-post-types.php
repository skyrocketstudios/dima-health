		...
		$order_statuses = apply_filters(
			...
			array(
				...
				'wc-arrival-shipment', array(
			        'label'                     => 'Approved by the Doctor',
			        'public'                    => false,
					'exclude_from_search'       => false,
					'show_in_admin_all_list'    => true,
					'show_in_admin_status_list' => true,
			        'label_count'               => _n_noop( 'Approved by the Doctor <span class="count">(%s)</span>', 'Approved by the Doctor <span class="count">(%s)</span>' )
			    ),
			)
		);
...
function register_approve_doctor_order_status() {
    register_post_status( 'wc-approve_doctor', array(
        'label'                     => 'Approved by the Doctor',
        'public'                    => false,
		'exclude_from_search'       => false,
		'show_in_admin_all_list'    => true,
...
add_action( 'init', 'register_approve_doctor_order_status' );

function add_approve_doctor_to_order_statuses( $order_statuses ) {
    $new_order_statuses = array();
    foreach ( $order_statuses as $key => $status ) {
        $new_order_statuses[ $key ] = $status;
        if ( 'wc-processing' === $key ) {
            $new_order_statuses['wc-approve_doctor'] = 'Approved by the Doctor';
        }
    }
    return $new_order_statuses;
}
add_filter( 'wc_order_statuses', 'add_approve_doctor_to_order_statuses' );

/* Add declined by the doctor */
function register_decline_doctor_order_status() {
    register_post_status( 'wc-decline_doctor', array(
        'label'                     => 'Declined by the Doctor',
        'public'                    => false,
		'exclude_from_search'       => false,
		'show_in_admin_all_list'    => true,
		'show_in_admin_status_list' => true,
        'label_count'               => _n_noop( 'Declined by the Doctor <span class="count">(%s)</span>', 'Approved by the Doctor <span class="count">(%s)</span>' )
    ) );
}
add_action( 'init', 'register_decline_doctor_order_status' );

function add_decline_doctor_to_order_statuses( $order_statuses ) {
    $new_order_statuses = array();
    foreach ( $order_statuses as $key => $status ) {
        $new_order_statuses[ $key ] = $status;
        if ( 'wc-processing' === $key ) {
            $new_order_statuses['wc-decline_doctor'] = 'Declined by the Doctor';
        }
    }
    return $new_order_statuses;
}
add_filter( 'wc_order_statuses', 'add_decline_doctor_to_order_statuses' );


/* Add ready for shipment */
function register_ready_shipment_order_status() {
    register_post_status( 'wc-ready_shipment', array(
        'label'                     => 'Ready for Shipment',
        'public'                    => false,
		'exclude_from_search'       => false,
		'show_in_admin_all_list'    => true,
		'show_in_admin_status_list' => true,
        'label_count'               => _n_noop( 'Ready for Shipment <span class="count">(%s)</span>', 'Ready for Shipment <span class="count">(%s)</span>' )
    ) );
}
add_action( 'init', 'register_ready_shipment_order_status' );

function add_ready_shipment_to_order_statuses( $order_statuses ) {
    $new_order_statuses = array();
    foreach ( $order_statuses as $key => $status ) {
        $new_order_statuses[ $key ] = $status;
        if ( 'wc-approve_doctor' === $key ) {
            $new_order_statuses['wc-ready_shipment'] = 'Ready for Shipment';
        }
    }
    return $new_order_statuses;
}
add_filter( 'wc_order_statuses', 'add_ready_shipment_to_order_statuses' );

/* Add shipment on the way */
function register_ongoing_shipment_order_status() {
    register_post_status( 'wc-ongoing_shipment', array(
        'label'                     => 'On-going Shipment',
        'public'                    => false,
		'exclude_from_search'       => false,
		'show_in_admin_all_list'    => true,
		'show_in_admin_status_list' => true,
        'label_count'               => _n_noop( 'On-going Shipment <span class="count">(%s)</span>', 'On-going Shipment <span class="count">(%s)</span>' )
    ) );
}
add_action( 'init', 'register_ongoing_shipment_order_status' );

function add_ongoing_shipment_to_order_statuses( $order_statuses ) {
    $new_order_statuses = array();
    foreach ( $order_statuses as $key => $status ) {
        $new_order_statuses[ $key ] = $status;
        if ( 'wc-ready_shipment' === $key ) {
            $new_order_statuses['wc-ongoing_shipment'] = 'On its way';
        }
    }
    return $new_order_statuses;
}
add_filter( 'wc_order_statuses', 'add_ongoing_shipment_to_order_statuses' );
