...
function wc_add_to_cart_message( $products, $show_qty = false, $return = false ) {
	...
	if ( 'yes' === get_option( 'woocommerce_cart_redirect_after_add' ) ) {
		...
		$message   = sprintf( '<p>%s</p> <a href="%s" tabindex="1" class="button wc-forward">%s</a> ', esc_html( $added_text ), esc_url( $return_to ), esc_html__( 'Continue shopping', 'woocommerce' ) );
	} else {
		$message = sprintf( '<p>%s</p> <a href="%s" tabindex="1" class="button wc-forward">%s</a> ', esc_html( $added_text ), esc_url( wc_get_cart_url() ), esc_html__( 'View cart', 'woocommerce' ) );
	}
