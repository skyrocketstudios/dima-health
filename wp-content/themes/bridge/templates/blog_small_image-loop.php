		...
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
				<?php if ( has_post_thumbnail() ) { ?>
					<div class="post_image ">
						<a itemprop="url" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
							<?php the_post_thumbnail('dima_blog_thumbnail'); ?>
						</a>
					</div>
				<?php } ?>
				<div class="post_text">
...
?>
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
				<?php if ( has_post_thumbnail() ) { ?>
					<div class="post_image" style="background:url('<?php echo get_the_post_thumbnail_url()?>')">
						<a itemprop="url" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
						</a>
					</div>
				<?php } else{ ?>
					<div class="post_image" style="background:url('<?php echo home_url()."/wp-content/uploads/2020/01/placeholder-600x400-1.png"?>')">
						<a itemprop="url" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>">
						</a>
					</div>
				<?php } ?>
				<div class="post_text">
					<div class="post_text_inner">
						<span class="custom_post_category">
							<?php 	$categories = get_the_category();
									if ( ! empty( $categories ) ) {
										echo esc_html( $categories[0]->name );   
										}?>
						</span>
						<h2 itemprop="name" class="entry_title"><meta itemprop="interactionCount" content="UserComments: <?php echo get_comments_number(bridge_qode_get_page_id()); ?>"/></span> <a itemprop="url" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php echo wp_trim_words(get_the_title(),8,'...'); ?></a></h2>
						<!-- <div class="post_info">
							<span class="time"><?php esc_html_e('Posted at','bridge'); ?> <?php the_time('H:i'); ?><?php esc_html_e('h','bridge'); ?></span>
							<?php esc_html_e('in','bridge'); ?> <?php the_category(', '); ?>
                        	<?php if($bridge_qode_blog_hide_author == "no") { ?>
						...
						<div class="custom_blog_post_excerpt">
							<?php echo	wp_trim_words(get_the_excerpt(),15,'...')	; 
								$guest_author = "";
								$guest_author = get_post_meta($post->ID,"author_name");
							?>
						</div>
						<span itemprop="dateCreated" class="date entry_date updated">by <?php echo $guest_author[0] != "" ? $guest_author[0]: the_author_meta('first_name'); ?> on <?php the_time('F j, Y'); ?>
					</div>
					<?php if ( ! post_password_required() ) { ?>
                            <div class="post_more">
                                <a itemprop="url" href="<?php the_permalink(); ?>" class="qbutton small custom_blog_button"><?php esc_html_e('Read More','bridge'); ?></a>
                            </div>
                    <?php } ?>
				</div>
			</div>
