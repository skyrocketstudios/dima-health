...
function my_custom_sidebar() {
    register_sidebar(
        array (
            'name' => __( 'Custom', 'dima.ph' ),
            'id' => 'custom-side-bar',
            'description' => __( 'Custom Sidebar', 'dima.ph' ),
...
if (!function_exists('bridge_qode_register_menus')) {
	...
	function bridge_qode_register_menus() {
		...
		add_theme_support('post-formats', array('asdide','gallery','link'));
...
if(!function_exists('bridge_qode_theme_setup')) {
    ...
    function bridge_qode_theme_setup() {
		...
		add_image_size( 'dima-blog-thumbnail', 450, 800, true );
...
function bbloomer_add_checkout_fee_for_gateway($order) {
	...
	// $zone_name = $shipping_zone->get_zone_name(); // G
	// var_dump($zone_name);
...
function bbloomer_refresh_checkout_on_payment_methods_change($order){
	foreach ( WC()->cart->get_cart() as $cart_item ) {
        $product = $cart_item['data'];
        if(!empty($product)){
			if($product->get_id() == 15943){
...
add_action( 'woocommerce_after_shop_loop_item', 'add_info_under_price_shipping_loop', 5 );

function add_info_under_price_shipping_loop() {
	global $product;
	if($product->get_id() == 5089){
		// Neo-Pen - 45,000+ boxes sold in 2019
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important;">45,000+ boxes sold in 2019</p>';
	}
	if($product->get_id() == 701){
		// Cybelle - 99,000+ boxes sold in 2019
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important; ">99,000+ boxes sold in 2019</p>';
	}
	if($product->get_id() == 708){
		// Liza - 22,000 + boxes sold in 2019
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important; ">22,000+ boxes sold in 2019</p>';
	}
	if($product->get_id() == 703){
		// Kadagra 50mg - 55,000+ boxes of 50mg and 100mg sold
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important; ">55,000+ boxes of 50mg and 100mg sold</p>';
	}
	if($product->get_id() == 706){
		// Kadagra 100mg - 55,000+ boxes of 50mg and 100mg sold
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important; "> 55,000+ boxes of 50mg and 100mg sold</p>';
	}
	if($product->get_id() == 699){
		// Protec Pills - 2.2 million+ boxes sold
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important; ">2.2 million+ boxes sold</p>';
	}
	if($product->get_id() == 713){
		// Protec Condoms - 16 million + boxes sold
		echo '<p style="color:#893146; text-align:center; font-size:12px; text-transform:uppercase; font-family:"gtamerica" !important;  ">16 million+ boxes sold</p>';
	}
...
// function get_wc_shipping_method( ){
//     global $wpdb;
...
function custom_shop_order_column( $columns )
{
    ...
    foreach( $columns as $key => $column ){
        ...
        if( 'order_status' == $key ){
            $ordered_columns['order_notes'] = __( 'Approved by', 'woocommerce');
        }
    }
           ...
           $notes = wc_get_order_notes( $args );
            ...
            if ( $notes ) {
                foreach( $notes as $key => $note ) {
                    if (strpos($note->content, 'to Approved by the Doctor.') !== false) {
                        $arr_auth[] = $note->added_by;
                    }
                }

                if($arr_auth !== NULL || !empty($arr_auth)) {
                    $last_doctor_approved = end($arr_auth);
                    echo $last_doctor_approved;
                } else {
...
function change_existing_currency_symbol( $currency_symbol, $currency ) {
     switch( $currency ) {
          case 'PHP': $currency_symbol = 'P '; break;
     }
     return $currency_symbol;
}

function add_user_logged_in_class_to_body( $classes ) {
...
add_filter( 'body_class','add_user_logged_in_class_to_body' );
...
function woo_add_cart_fee() {
  ...
  if(is_user_logged_in()==true){
        ...
        global $wpdb;
        $user_stats = $wpdb->get_results( "SELECT * FROM $wpdb->usermeta WHERE user_id =$user and meta_key='fee_status'"); 
        
       if(!empty($user_stats)) {
            foreach($user_stats as $row){
                 $stats = $row->meta_value;
                if($stats== 1) {
                ...
                $status=0;
                add_user_meta( $user, 'fee_status', $status);
         }


       $customer_id = $wpdb->get_results( "SELECT customer_id,user_id,email FROM dima_wc_customer_lookup WHERE user_id =$user"); 
        if(!empty($customer_id)) {
           foreach($customer_id as $row){
                $cust_id = $row->customer_id;  
           }
           $order_stats = $wpdb->get_results( "SELECT * FROM dima_wc_order_stats WHERE customer_id =$cust_id"); 
           foreach($order_stats as $row){
                $stats = $row->status;  
                if($stats=="wc-completed" || $stats=="wc-approve_doctor"){
                    $status=1;
                update_user_meta( $user, 'fee_status', $status);
                }
           }
        }
...
function my_custom_action() { 
    echo '<div id="alg_checkout_files_upload_form_1">
    <table>
        <tbody>
            <tr>
                <td colspan="2">
                    <span onclick="showHideInput()">If you have an existing prescription,  
                        <span class="text_secondary">upload it here!
                        </span>
                    </span>
                </td>
            </tr>
            <tr id="file_input_2" style="display: none;">
                <td colspan="2">
                    <div style="margin-bottom:5px;">
                        <input type="file" class="alg_checkout_files_upload_file_input" file-num="1" name="alg_checkout_files_upload_1" id="alg_checkout_files_upload_1" accept=".jpg,.jpeg,.png" style="" required>
                    </div>
                    <div id="alg_checkout_files_upload_result_1" style="display:none;margin-bottom:5px;">
                        <span id="alg_checkout_files_upload_result_file_name_1">	
                        </span> 
                        <a href="" class="alg_checkout_files_upload_result_delete" file-num="1" style="color:red;" title="Remove">×</a>
                    </div>
                </td>
            </tr>
        </tbody>
    ...
    <input type="hidden" id="alg_checkout_files_upload_order_id_1" name="alg_checkout_files_upload_order_id_1" value="0">
    <input type="hidden" id="alg_checkout_files_upload_order_key_1" name="alg_checkout_files_upload_order_key_1" value="0">
</div>';
};     
...
add_filter( 'woocommerce_admin_order_data_after_billing_address', 'custom_checkout_field_display_admin_order_meta', 10, 1 );
...
function custom_checkout_field_display_admin_order_meta($order){
    ...
    if( $order->get_used_coupons() ) {
		...
		echo '<p><strong>' . 'No coupon used.' . '</strong> ';
		echo '</p>';
	}
...
// if shipping zone is changed 

// add_filter('woocommerce_before_checkout_shipping_form','customChangeShippingState');
// function customChangeShippingState(){
		
// }


add_action( 'woocommerce_admin_order_data_after_billing_address', 'custom_edit_next_prev_order_button', 10);
function custom_edit_next_prev_order_button(){
	$current_id = get_the_ID();
	$orders_id = wc_get_orders(array(
		'numberposts' => -1,
		'return' => 'ids',
	));
	$next_order_details = "";
	...
	foreach($orders_id as $order_id){
		if($current_id == $order_id){
			...
			$current_order_id =$orders_id[$count];
			if($orders_id[$count-1] == 0){
				$prev_order_details = 0;
			}else{
				$prev_order_details = $orders_id[$count-1];
			}
			if($orders_id[$count+1] == 0){
				$next_order_details = 0;
			}else{
				$next_order_details = $orders_id[$count+1];
			}
		}else{
			$count++;
		}
	...
	if($screen->post_type == "shop_order" && $screen->id == "shop_order" && $screen->parent_file == "woocommerce" && $current_id == $current_order_id){
		// $getLink = get_edit_post_link($prev_order_details);
		if($prev_order_details > 0 && $next_order_details > 0){
			echo '<a href=" '.get_edit_post_link($prev_order_details).' " class="prev-post components-button editor-post-preview is-button is-primary is-large">&larr; Previous</a>';
			echo '<a href=" '.get_edit_post_link($next_order_details).' " class="next-post components-button editor-post-preview is-button is-primary is-large">Next &rarr;</a>';
		}else if($prev_order_details == 0 && $next_order_details > 0){
			echo ' <a href=" '.get_edit_post_link($next_order_details).' " class="next-post components-button editor-post-preview is-button is-primary is-large">Next &rarr;</a>';
		}else if($prev_order_details > 0 && $next_order_details == 0){
			echo '<a href=" '.get_edit_post_link($prev_order_details).' " class="prev-post components-button editor-post-preview is-button is-primary is-large">&larr; Previous</a>';
		}
...
// add_filter('woe_get_order_value_coupon_used', function ($value, $order) {
// 	if( $order->get_used_coupons() ) {
// 		$value="";
// 		$coupons_count = count( $order->get_used_coupons() );
// 		$i = 1;
...
// add_filter('woe_get_order_value_cart_coupon',function($order,$value){
// 		if( $order->get_used_coupons() ) {
// 			$coupons_count = count( $order->get_used_coupons() );
// 			$i = 1;
// 			$coupon="";
	...
	function widget($args, $instance) {
		...
		if( $r->have_posts() ) :
			...
			<?php foreach ( $r->posts as $recent_post ) : ?>
				...
				$aria_current = '';
				$categories = get_the_category( $recent_post->ID)[0]->name;
				// if ( ! empty( $categories ) ) 
				// {
				...
				?>
				<li class="custom_recent_posts">
					<div><a href="#" class="category" ><?php if($categories){ echo $categories; }?></a></div>
					<a href="<?php the_permalink( $recent_post->ID ); ?>"<?php echo $aria_current; ?>><?php echo $title; ?></a>
					<p><?= wp_trim_words($recent_post->post_content,40,'...') ?></p>
					<?php if ( $show_date ) : ?>
